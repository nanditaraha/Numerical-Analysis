/*
 Author - Nandita Raha
 Usage: root -l
 .x simulate_gain.C(parameter_list)
 Or load the macro
 */
#include <TH1D.h>
#include <TFile.h>
...
//This function same name as should initialize all common parameters
void simulate_gain_SLAC_2016();
void wiggle(double eps, double tau, double tau_r);
void wiggle_correct(double eps, double tau, double tau_r);
void simulate_gain_SLAC_2016()//will define later
{}
void simulate(Int_t events=2000, Int_t rep=1){
    ...
    //ATF gain function
    TF1* f_gain1 = new TF1("f_gain1","1/[0] -3*[1]*(TMath::Exp(-x/[2]) - TMath::Exp(-x/[3]))",0,time*turn);
    TF1* f_gain = new TF1("f_gain","1 -3*[0]*(TMath::Exp(-x/[1]) - TMath::Exp(-x/[2]))",0,time*turn);
    ...
    //ATF gain function
    f_gain->SetParNames("eps","#tau_{#mu}","#tau_{r}");
    f_gain1->SetParNames("PMT gain","eps","#tau_{#mu}","#tau_{r}");
    ...
    f_gain1->SetParameters(1.01,eps,64400,3000);//ATF
    //f_gain1->SetParameters(1,eps,64400);//exp
    f_gain->SetParameters(eps,64400,3000);//ATF
    ...
    for(int i=1;i<=280;i++){//time bins (gaps in time of 5 us)
        ...
        for (Int_t j = 0; j < events; j++) {//Loop of 2000 simulated points to produce ideal gain
            mean = f_gain->Eval(i*7e5/280);//f_gain is our SiPM exponential function at 5, 10, 15... us i.e 5 us or 5000 ns apart
    ...
    for(int i=1;i<=140;i++){//time bins (gaps in time of 5 us)
        ...
        for (Int_t j = 0; j < events+1; j++) {//Loop of 2000 simulated points to produce ideal gain
            //pmt=1+1e-5*j;//PMT gain is 2% for 2000 cycles. We use gain for 1 cycle = 1e-5
            //printf("PMT %f\n",pmt);
            ...
            //pmt=1.01+0.001*j/2000.;//0.1% //skipping 10 events
    ...
    //wiggle(f_gain->GetParameter(0),f_gain->GetParameter(1),f_gain->GetParameter(2));//for fitting h_gain_norm
    //wiggle(f_gain1->GetParameter(1),f_gain1->GetParameter(2));//for fitting h_gain
    //wiggle(0.001,64400);
    //TCanvas* c2 = new TCanvas("c2");;
    //wiggle_correct(f_gain1->GetParameter(1),f_gain1->GetParameter(2)+f_gain1->GetParError(2));// add subtract error
    //wiggle_correct(f_gain->GetParameter(0),f_gain->GetParameter(1),f_gain->GetParameter(2));
...
void wiggle(double eps, double tau, double tau_r) {
    ...
    for(int it=0; it<time;it++){
        ...
        gain = 1 + (-3*eps*(TMath::Exp(-(it*turn)/tau) - TMath::Exp(-(it*turn)/tau_r))); //ATF gain function
...
void wiggle_correct(double eps, double tau, double tau_r)
    {
        ...
        for(int it =0; it<time;it++)
        {
            ...
            double gain = 1. + (-3*eps*(TMath::Exp(-(it*turn)/tau) - TMath::Exp(-(it*turn)/tau_r)));
        ...
        for(int it=0; it<time;it++){
            ...
            gain = 1 + (-3*eps*(TMath::Exp(-(it*turn)/tau1) - TMath::Exp(-(it*turn)/3000.))); //ATF gain function - in theory ATF tau_r is 3000 ns
            //Theoretical gain perturbation
            for(int i=0.6*bin;i<bin;i++){//threshold is at y=0.6
                Double_t Nytot=0, Nytot1=0;
                for(int j=0; j<bin; j++){//this loop is for reconstruction considering probability distributions...
