...
Double_t prob_ideal[1000][1000]={0};
Double_t T[500]={0};
...
void wiggle(int n=50, double start=0.) {
  ...
  Double_t gain = 1., gain1=1, gain2=1;
  ...
  TFile *f = TFile::Open("long_gain.root");
  ...
  TH1D* h_wiggletot = new TH1D("Stable Case","Stable Case",time, 0., time*turn);
  //TH1D* h_wiggletot = new TH1D("New linear-[1+  #epsilon (T-t)/T]","New linear-[1+  #epsilon (T-t)/T]",time, 0., time*turn);
  TH1D* h_wiggletot_exp = new TH1D("New exp-[1+  #epsilone^{-(t/#tau)}]","New exp-[1+  #epsilone^{-(t/#tau)}]",time, 0., time*turn);
  TH1D* h_wiggletot_cos = new TH1D("New phase-[1+  #epsilonCos(#omegat+#phi)]","New phase-[1+  #epsilonCos(#omegat+#phi)]",time, 0., time*turn);
  ...
  for(int it=0; it<time;it++){
    ...
    gain=h_gain_final->GetBinContent(it);
    ...
    //gain1 = 1. + (eps*TMath::Exp((-it*turn)/(tau))); //exponential gain fluctutation varying tau - plan later
    ...
    for(int i=0.6*bin;i<bin;i++){// 60 is threshold energy for 100 bins - thus used 0.6*bin for increasing bins
	...
	//prob = (-TMath::Erfc((((float(i+1))/bin)-y)/resolution)+TMath::Erfc(((float(i)/bin)-y)/resolution))/(2.);
	//prob1 = (TMath::Erf((((float(i+1))/bin)-y1)/resolution)-TMath::Erf(((float(i)/bin)-y1)/resolution))/(2.);
	//prob2 = (TMath::Erf((((float(i+1))/bin)-y2)/resolution)-TMath::Erf(((float(i)/bin)-y2)/resolution))/2.;// I use this
	...
	//prob = (-TMath::Erfc((((float(i+1))/bin)-mean)/resolution)+TMath::Erfc(((float(i)/bin)-mean)/resolution))/(2.);
        ...
        //prob2 = (-TMath::Erfc((((float(i+1))/bin)-mean2)/resolution2)+TMath::Erfc(((float(i)/bin)-mean2)/resolution2))/(2.);
	
	Na = hn->GetBinContent(int(gain*j)); 
	A = ha->GetBinContent(int(gain*j));  
	phi = hp->GetBinContent(int(gain*j));
	
	Na1 = hn->GetBinContent(int(gain1*j));
        A1 = ha->GetBinContent(int(gain1*j));
        phi1 = hp->GetBinContent(int(gain1*j));

	Na2 = hn->GetBinContent(int(gain2*j));
        A2 = ha->GetBinContent(int(gain2*j));
        phi2 = hp->GetBinContent(int(gain2*j));	
  ...
  //h_prob->Draw();
  //c1->cd(2);
  //    h_wiggletot_    h_wiggletot_cos->SetBinContent(it,Nt2);
  ...
  */
  ...
  //SCAn fit parameters just to check if it minimizes ok - index starts from 1
  /*
  gMinuit->Command("SCAn 4 100");
