...
//double dropE = 6.4e-5;//for 150 MeV positrons
...
void gainSimulationTest(int cycle,int fill, int laser) {
  ...
  */
  ...
  for(int c=0;c<cycle;c++){
	...
	double drop = Prob->GetRandom()*dropE/100.;
	//cout<<"Energy "<<drop<<"\n";
  ...
  TFile *file1 = TFile::Open(Form("varyDrop_fac%d_m%d_c%d_f%d_%d_%d.root",factor,nMuon,cycle, fill, gap, offset), "RECREATE");
