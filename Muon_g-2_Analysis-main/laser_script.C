/*
 ...
 An instance of gainDropInFill (drop here) calls operator() which is a function.
 
 Laser: 
 drop
Function gainSimulation() draws the function for 100 muons of a fill (just one fill)
I modify this with parameters for everything so I can put it in a shell script
  */
...
//Use a scale of dropE for 1500 MeV positrons
//double dropE = 6.4e-4;//for 1500 MeV positrons
int factor=1;
//Here we change to laser energy corresponding to 100 MeV
//double dropE = factor*4.267e-5;//for 100 MeV positrons - 4.267e-5 works best - standard
double dropE = factor*4.267e-6;//for plots drop scan, with energy from 0.1*alpha or 100 MeV
double tau_r=18;
...
void gainSimulationTest(int cycle,int fill, int laser, int nMuon, int factor, int gap) {
  //gRandom->SetSeed(0);
  double mean,sigma,data,err,data1;
  TGraph* g = new TGraph();
  cout<<"Number of muons %d"<<nMuon<<"\n";
  TF1* f;
  TF1* f1;
  TFile *file = TFile::Open("prob.root");
  TH1 *Prob = (TH1*)file->Get("Prob");
  dropE = factor*4.267e-6; //i forgot this in the script :(
  ...
  //TF1 *t1 = new TF1("t1","exp(-x/64.4)",0,700);
  //t1->SetNpx(cycle*fill*nMuon);
  ...
  for(int c=0;c<cycle;c++){
	...
	gRandom->SetSeed(0);
	double drop = Prob->GetRandom()*dropE/100.;//when we use alpha def and scan it.
	//cout<<"Energy "<<drop<<"\n";
	...
	dropPerDecay.push_back(dropE*20);// Assume this for 2 GeV laser shots - for alpha def and scan.
	...
	//cout<<"No of samples:"<<decayTimes_sort.size()<<"="<<decayTimes_sort[i]<<" Gain="<<f->Eval(decayTimes_sort[i])<<" E drop="<<dropPerDecay_sort[i]<<"\n";
	/*
	if(decayTimes_sort[i]==){//Lasers only
	  //cout<<"No of samples:"<<decayTimes_sort.size()<<"="<<decayTimes_sort[i]<<" Gain="<<f->Eval(decayTimes_sort[i])<<" E drop="<<dropPerDecay_sort[i]<<"\n";
	  xhist3->Fill(decayTimes_sort[i],f1->Eval(decayTimes_sort[i])); 
	  hist1->SetBinContent(hist1->FindBin(decayTimes_sort[i])-1,f1->Eval(decayTimes_sort[i])); 
	  }*/
  ...
  latex.DrawLatex(121.5,.992,Form("p_{0} = P_{0} #tau_{#mu} = %0.1f #times 10^{-4}",6.4));
  latex.DrawLatex(121.5,.990,Form("Drop per 100 MeV, per pulse = %0.1f #times 10^{-5}",dropE*1e5));
  ...
  */
  ...
  //TFile *file1 = TFile::Open(Form("try_c%d_%d_%d.root",cycle,gap,offset), "RECREATE");
  hf->Write("Laser");
  hf1->Write("Muons");
  hf2->Write("All");
  printf("End run fac=%d gap=%d\n",factor,gap);
  //file1->Delete("hf");
