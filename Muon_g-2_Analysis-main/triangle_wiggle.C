/* Changed gain function to triangular shape
 Used units of us instead of ns
 */
#include <TH1D.h>
#include <TFile.h>
#include <TNtuple.h>
...
void wiggle(double width,int n, Double_t start=0.0, Double_t eps=0.001) {
//Variables
  ...
  Double_t fNt;
  Double_t gain = 0.;
  Double_t turn = 0.149; //considering 1 turn as 149 ns
  ...
  Double_t time = 4000; //600 us if turn 149 ns
...
//   Nfunc->SetParameters(Norm);
  ...
  TH1D* hn = new TH1D("hn","N(y)",100,0.,1.); 
  hn->Eval(Nfunc);
...
//phase function
  ...
  TH1D* hp = new TH1D("hp","Phi(y)",100,0.,1.);
  hp->Eval(phase);
...
//Fit Function
  TF1* fit_func1 = new TF1("fit_func1","[0]*(1+[2]*cos(1.439*(1-[4]*1.e-6)*x+([3]+2*pi)))*exp(-x/[1])",0.0,time*turn);
  //TF1* fit_func1 = new TF1("fit_func1","[0]*(1+[2]*cos(0.001439*(1-[4]*1.e-6)*x+([3]+2*pi)))*exp(-x/[1])",start,time*turn);
...
for(int it=0; it<time;it++){
  ...
  Double_t del = rd.Gaus(0.,1.);
    ...
    if(start==0.0)
    {
        if(it*turn<width) gain=1+eps*(it*turn)/(time*turn);
        //if(it*turn>(25+start) && it*turn<(50+start)) gain=1-eps*((it-start)*turn)/(time*turn) + 2*eps*(25.-start)/(time*turn);
        if(it*turn>width && it*turn<2*width) gain=1-eps*((it*turn)/(time*turn)) + 2*eps*width/(time*turn);
        if(it*turn>2*width) gain = 1;

    }
else
 {
    if(it*turn<start) gain=1;
    if(it*turn>start && it*turn<(width+start)) gain=1+eps*(it*turn)/(time*turn) - eps*start/(time*turn);
    //if(it*turn<(25+start)) gain=1+eps*((it+start)*turn)/(time*turn);
     if(it*turn>(width+start) && it*turn<(2*width+start)) gain=1-eps*((it*turn)/(time*turn)) + eps*(start+2*width)/(time*turn);
    if(it*turn>(2*width+start)) gain = 1;
 }
    //gain=1+eps*(it*turn)/(time*turn);
    for(int i=60;i<bin;i++){
	  ...
	  //Double_t Ny = Na*(1+A*TMath::Cos(0.001439*it*turn + phi*1.e-3))*prob;
        Double_t Ny = Na*(1+A*TMath::Cos(1.439*it*turn + phi*1.e-3))*prob;
	  Nytot = Nytot+Ny;
        } //end for j
        Ntot = Ntot+Nytot;
      }//end for i
...
//========================================================================================================================================================
   ...
   h_wiggletot->SetXTitle("t[#mu s]");
   h_wiggletot->SetYTitle("Entries");
   gStyle->SetOptFit(1);
    
   R_w[n]=fit_func1->GetParameter(4)-(-2.70923e-02);
   //printf("delta_w %f\n",(fit_func1->GetParameter(4)-(-2.70923e-02)));
...
void startScan(double width=25,int n=60, double eps=0.001){
    double x[500]={0};
    for(int i=0; i<n; i++){
        x[i]= i*10; //as bin size is 149
        wiggle(width,i,x[i],eps);
    ...
    */
    ...
    TGraph *range_R = new TGraph(n,x,R_w);
    range_R->GetXaxis()->SetTitle("Time ( #mu s)");
    range_R->GetXaxis()->CenterTitle();
    range_R->GetYaxis()->SetTitle("#delta#omega ");
